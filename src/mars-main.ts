...
document.addEventListener('pointerover', e => {const button=(e.target as Element).closest?.('button') as HTMLButtonElement|null;if(button)showTip(button);});
...
canvas.addEventListener('pointerdown', e => {if (e.button !== 0 || view.mode === 2 || view.mode === 8) return; Object.assign(stick, {active: true, ox: e.clientX, oy: e.clientY, x: 0, y: 0}); const s = $('stick'); s.style.left = `${e.clientX}px`; s.style.top = `${e.clientY}px`; s.classList.add('on');});
addEventListener('pointermove', e => {if (!stick.active) return; const dx = (e.clientX - stick.ox) / 110, dy = (e.clientY - stick.oy) / 110, l = Math.hypot(dx, dy), k = l > 1 ? 1 / l : 1; stick.x = dx * k; stick.y = dy * k; ($('stick').firstElementChild as HTMLElement).style.transform = `translate(${stick.x * 44}px,${stick.y * 44}px)`;});
const release = () => {stick.active = false; stick.x = stick.y = 0; $('stick').classList.remove('on');};
